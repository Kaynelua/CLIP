...
def visualize_ranked_results_text(distmat,queryList, width=128, height=256, save_dir='data/places365_large/viz', topk=10):
    """Visualizes ranked results.
    ...
    for q_idx in range(num_q):
        ...
        ##Additional code added here:##
        textQuery = queryList[q_idx]
        qpid = q_idx

        ####

        qimg = 255 * np.ones((height, width, 3),dtype=np.uint8)
        cv2.putText(qimg,textQuery ,(10,10),cv2.FONT_HERSHEY_SIMPLEX,0.3,(0,0,0))
        qimg = cv2.copyMakeBorder(qimg, BW, BW, BW, BW, cv2.BORDER_CONSTANT, value=(0, 0, 0))
        # resize twice to ensure that the border width is consistent across images
        qimg = cv2.resize(qimg, (width, height))
        num_cols = topk + 1
        ...
        classID = qpid
        cv2.imwrite(osp.join(save_dir, str(qpid)+'.jpg'), grid_img)
